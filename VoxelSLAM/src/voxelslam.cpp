...
public:
  ...
  void pub_localmap(int mgsize, int cur_session, vector<PVecPtr> &pvec_buf,
                    vector<IMUST> &x_buf, pcl::PointCloud<PointType> &pcl_path,
  ...
  void pub_global_path(vector<vector<ScanPose *> *> &relc_bl_buf,
                       ros::Publisher &pub_relc, vector<int> &ids) {
  ...
  void pub_globalmap(vector<vector<Keyframe *> *> &relc_submaps,
                     vector<int> &ids, ros::Publisher &pub) {
    pcl::PointCloud<pcl::PointXYZI> pl;
    pub_pl_func(pl, pub);
    pcl::PointXYZI pp;
...
public:
  ...
  void save_pose(vector<ScanPose *> &bbuf, string &fname, string posename,
                 string &savepath) {
    if (bbuf.size() < 100)
      return;
  ...
  void pgo_edges_io(PGO_Edges &edges, vector<string> &fnames, int io,
                    string &savepath, string &bagname) {
    ...
    if (io == 0) // read
    {
      ...
      while (getline(infile, lineStr)) {
        ...
        if (mp[0] != -1 && mp[1] != -1) {
          ...
          Eigen::Quaterniond qq(stod(sts[10]), stod(sts[7]), stod(sts[8]),
                                stod(sts[9]));
  ...
  void previous_map_names(ros::NodeHandle &n, vector<string> &fnames,
                          vector<double> &juds) {
    string premap;
    n.param<string>("General/previous_map", premap, "");
    premap.erase(remove_if(premap.begin(), premap.end(), ::isspace),
                 premap.end());
  ...
  void previous_map_read(vector<STDescManager *> &std_managers,
    ...
    for (int fn = 0; fn < fnames.size() && n.ok(); fn++) {
      ...
      for (int i = 0; i + acsize < subsize && n.ok(); i += mgsize) {
        ...
        vector<STD> stds_vec;
        std_manager->GenerateSTDescs(pl_btc, stds_vec,
                                     keyframes_tem->at(up - 1)->id);
        std_manager->AddSTDescs(stds_vec);
      }
      std_manager->config_setting_.skip_near_num_ =
          -(std_manager->plane_cloud_vec_.size() + 10);
...
public:
  ...
  void motion_blur(pcl::PointCloud<PointType> &pl, PVec &pvec, IMUST xc,
                   IMUST xl, deque<sensor_msgs::Imu::Ptr> &imus,
                   double pcl_beg_time, IMUST &extrin_para) {
    xc.bg = xl.bg;
    xc.ba = xl.ba;
    ...
    for (auto it_imu = imus.end() - 1; it_imu != imus.begin(); it_imu--) {
      ...
      imu_poses.emplace_back(offt, R_imu, pos_imu, vel_imu, angvel_avr,
                             acc_imu);
    }
    ...
    for (auto it_kp = imu_poses.begin(); it_kp != imu_poses.end(); it_kp++) {
      ...
      for (; it_pcl->curvature > head.t; it_pcl--) {
        ...
        Eigen::Vector3d T_ei =
            pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt - xc.p;
  ...
  int motion_init(vector<pcl::PointCloud<PointType>::Ptr> &pl_origs,
                  ...
                  vector<double> &beg_times, Eigen::MatrixXd *hess,
    ...
    for (int iterCnt = 0; iterCnt < 10; iterCnt++) {
      ...
      for (int i = 0; i < win_size; i++) {
        ...
        pvec_buf[i]->clear();
        int l = i == 0 ? i : i - 1;
        motion_blur(*pl_origs[i], *pvec_buf[i], x_buf[i], x_buf[l], vec_imus[i],
                    beg_times[i], extrin_para);
      ...
      if (fabs(resis[0] - resis[1]) / resis[0] < converge_thre &&
          iterCnt >= 2) {
    ...
    Eigen::Vector3d acc(vec_imus[0][0]->linear_acceleration.x,
                        vec_imus[0][0]->linear_acceleration.y,
...
public:
  ...
  VOXEL_SLAM(ros::NodeHandle &n) {
    ...
    if (feat.lidar_type == LIVOX)
      sub_pcl = n.subscribe<livox_ros_driver::CustomMsg>(lid_topic, 1000,
                                                         pcl_handler);
    else
      sub_pcl =
    ...
    odom_ekf.Lid_rot_to_IMU << vecR[0], vecR[1], vecR[2], vecR[3], vecR[4],
        vecR[5], vecR[6], vecR[7], vecR[8];
    ...
    n.param<vector<double>>("LocalBA/plane_eigen_value_thre",
    ...
    noiseMeas.setZero();
    noiseWalk.setZero();
    noiseMeas.diagonal() << cov_gyr, cov_gyr, cov_gyr, cov_acc, cov_acc,
        cov_acc;
    ...
    for (int iterCount = 0; iterCount < num_max_iter; iterCount++) {
      ...
      for (int i = 0; i < psize; i++) {
        ...
        if (octos[i] != nullptr && octos[i]->inside(wld)) {
          double max_prob = 0;
          flag =
              octos[i]->match(wld, pla, max_prob, var_world, sigma_d, octos[i]);
        } else {
      ...
      Eigen::Matrix<double, DIM, 1> solution =
          K_1.block<DIM, 6>(0, 0) * HTz + vec -
          G.block<DIM, 6>(0, 0) * vec.block<6, 1>(0, 0);
      ...
      if (flg_EKF_converged ||
          ((rematch_num == 0) && (iterCount == num_max_iter - 2))) {
        rematch_num++;
      }
  ...
  void loop_update() {
    ...
    printf("scanPoses: %zu %zu %zu %d %d %zu\n", scanPoses->size(),
           buf_lba2loop.size(), x_buf.size(), win_base, win_count,
           sws[0].size());
  ...
  int initialization(deque<sensor_msgs::Imu::Ptr> &imus, Eigen::MatrixXd &hess,
                     ...
                     pcl::PointCloud<PointType>::Ptr pcl_curr) {
    ...
    if (win_count > 1) {
      imu_pre_buf.push_back(
          new IMU_PRE(x_buf[win_count - 2].bg, x_buf[win_count - 2].ba));
  ...
  void multi_margi(unordered_map<VOXEL_LOC, OctoTree *> &feat_map, double jour,
    ...
    auto margi_func = [](int win_cnt, vector<OctoTree *> *oct,
                         vector<IMUST> xxs, LidarFactor &voxhess) {
      for (OctoTree *oc : *oct) {
  ...
  void thd_odometry_localmapping(ros::NodeHandle &n) {
    ...
    while (n.ok()) {
      ...
      if (win_count >= win_size) {
        ...
        ofs << std::fixed;
        ofs << x_curr.t << " " << x_curr.p.x() << " " << x_curr.p.y() << " "
            << x_curr.p.z() << " " << q.x() << " " << q.y() << " " << q.z()
            << " " << q.w() << std::endl;
        ...
        if (is_save_map) {
          for (int i = 0; i < mgsize; i++)
            FileReaderWriter::instance().save_pcd(
                pvec_buf[i], x_buf[i], win_base + i, savepath + bagname);
        }
  ...
  void build_graph(gtsam::Values &initial, gtsam::NonlinearFactorGraph &graph,
    ...
    for (int i = 0; i < ids.size(); i++)
      stepsizes.push_back(stepsizes.back() +
                          multimap_scanPoses[ids[i]]->size());
    ...
    for (int ii = 0; ii < ids.size(); ii++) {
      ...
      for (int j = bsize; j < stepsizes[ii + 1]; j++) {
        ...
        if (j > bsize) {
          ...
          add_edge(j - 1, j, multimap_scanPoses[id]->at(j - 1 - bsize)->x,
                   multimap_scanPoses[id]->at(j - bsize)->x, graph, v6_noise);
          // add_edge(j-1, j, multimap_scanPoses[id]->at(j-1-bsize)->x,
          // multimap_scanPoses[id]->at(j-bsize)->x, graph, default_noise);
        }
      }
    }
    ...
    if (multimap_scanPoses[ids[0]]->size() != 0) {
      ...
      for (int i = 0; i < ceil; i++) {
        ...
        gtsam::Pose3 pose3 =
            gtsam::Pose3(gtsam::Rot3(xf.R), gtsam::Point3(xf.p));
      ...
      for (PGO_Edge &edge : lp_edges.edges) {
        ...
        if (edge.is_adapt(ids, step)) {
          ...
          for (int i = 0; i < edge.rots.size(); i++) {
            ...
            add_edge(id1, id2, edge.rots[i], edge.tras[i], graph,
                     default_noise);
          }
        }
      }
  ...
  void thd_loop_closure(ros::NodeHandle &n) {
    ...
    FileReaderWriter::instance().pgo_edges_io(lp_edges, sessionNames, 0,
                                              savepath, bagname);
    FileReaderWriter::instance().previous_map_read(
        std_managers, multimap_scanPoses, multimap_keyframes, config_setting,
        lp_edges, n, sessionNames, juds, savepath, win_size);
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr plbtc(
        new pcl::PointCloud<pcl::PointXYZI>);
    IMUST x_key;
    int buf_base = 0;
    ...
    while (n.ok()) {
      if (reset_flag == 1) {
        reset_flag = 0;
        scanPoses->insert(scanPoses->end(), buf_lba2loop_tem.begin(),
                          buf_lba2loop_tem.end());
        ...
        ResultOutput::instance().pub_global_path(multimap_scanPoses,
                                                 pub_prev_path, ids);
        ResultOutput::instance().pub_globalmap(multimap_keyframes, ids,
                                               pub_pmap);
      ...
      if (g_pos > 0) {
        ...
        gtsam::noiseModel::Diagonal::shared_ptr v6_noise =
            gtsam::noiseModel::Diagonal::Variances(samv6);
        add_edge(g_pos - 1, g_pos, scanPoses->at(buf_base - 1)->x, xc, graph,
                 v6_noise);
      } else {
        gtsam::Pose3 pose3(gtsam::Rot3(xc.R), gtsam::Point3(xc.p));
      ...
      for (int id = 0; id <= cur_id; id++) {
        std_managers[id]->SearchLoop(stds_vec, search_result, loop_transform,
                                     loop_std_pair,
        ...
        if (search_result.first >= 0 && search_result.second > juds[id]) {
          if (icp_normal(
            ...
            int ord_bl = std_managers[id]
                             ->plane_cloud_vec_[search_result.first]
                             ->header.seq;
            ...
            if (id == cur_id) {
              double span =
                  smp->jour - keyframes->at(search_result.first)->jour;
              printf("drift: %lf %lf\n", drift_p, span);
            ...
            if (isPush) {
              match_num++;
              lp_edges.push(id, cur_id, ord_bl, buf_base - 1,
                            loop_transform.second, loop_transform.first,
                            v6_init);
              if (step > -1) {
                int id1 = stepsizes[step] + ord_bl;
                int id2 = stepsizes.back() - 1;
                add_edge(id1, id2, loop_transform.second, loop_transform.first,
                         graph, odom_noise);
      ...
      if (isGraph) {
        build_graph(initial, graph, cur_id, lp_edges, odom_noise, ids,
                    stepsizes, 1);
      }
      ...
      if (isOpt) {
        ...
        for (int ii = 0; ii < idsize; ii++) {
          ...
          for (int j = stepsizes[ii]; j < stepsizes[ii + 1]; j++) {
            int ord = j - stepsizes[ii];
            multimap_scanPoses[tip]->at(ord)->set_state(
                results.at(j).cast<gtsam::Pose3>());
          }
        }
        mtx_keyframe.lock();
        for (int ii = 0; ii < idsize; ii++) {
        ...
        ResultOutput::instance().pub_global_path(multimap_scanPoses,
                                                 pub_prev_path, ids2);
        ResultOutput::instance().pub_globalmap(multimap_keyframes, ids2,
                                               pub_pmap);
        ids2.clear();
    ...
    if (is_finish) {
      ...
      int cur_id = std_managers.size() - 1;
      build_graph(initial, graph, cur_id, lp_edges, odom_noise, ids, stepsizes,
                  0);
    ...
    if (is_save_map) {
      for (int i = 0; i < ids.size(); i++)
        FileReaderWriter::instance().save_pose(*(multimap_scanPoses[ids[i]]),
                                               sessionNames[ids[i]],
                                               "/alidarState.txt", savepath);

      FileReaderWriter::instance().pgo_edges_io(lp_edges, sessionNames, 1,
                                                savepath, bagname);
    }
  ...
  void topDownProcess(gtsam::Values &initial,
                      gtsam::NonlinearFactorGraph &graph, vector<int> &ids,
  ...
  void HBA_add_edge(vector<IMUST> &p_xs, vector<Keyframe *> &p_smps,
                    PGO_Edges &gba_edges, vector<int> &maps, int max_iter,
                    int thread_num,
    ...
    for (int iterCnt = 0; iterCnt < max_iter; iterCnt++) {
      ...
      vector<double> resis;
      bool is_converge =
          opt_lsv.damping_iter(xs, voxhess, &hess, resis, up, is_display);
      if (is_display)
        printf("%lf\n", fabs(resis[0] - resis[1]) / resis[0]);
      if ((fabs(resis[0] - resis[1]) / resis[0] < converge_thre &&
           is_converge) ||
          (iterCnt == max_iter - 2 && converge_flag == 0)) {
        converge_thre = 0.01;
        if (converge_flag == 0) {
          converge_flag = 1;
  ...
  void thd_globalmapping(ros::NodeHandle &n) {
    ...
    n.param<vector<double>>("GBA/eigen_value_array", gba_eigen_value_array,
                            vector<double>());
    ...
    while (n.ok()) {
      ...
      int smp_flag = 0;
      if (smp_mp + 1 < multimap_keyframes.size() &&
          !multimap_keyframes.back()->empty())
        smp_flag = 1;
      ...
      if (total_ba == 1) {
        ...
        HBA_add_edge(xs, gba_submaps, gba_edges2, cnct_map, total_max_iter,
                     thread_num);
        ...
        gba_flag = 0;
                 ...
                 multimap_keyframes[smp_mp]->size() <= buf_base) {
        smp_mp++;
        buf_base = 0;
        localID.clear();
        // printf("switch: %d\n", smp_mp);
