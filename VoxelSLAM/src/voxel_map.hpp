...
void down_sampling_pvec(PVec &pvec, double voxel_size,
                        pcl::PointCloud<PointType> &pl_keep) {
...
void Bf_var(const pointVar &pv, Eigen::Matrix<double, 9, 9> &bcov,
            const Eigen::Vector3d &vec) {
  Eigen::Matrix<double, 6, 3> Bi;
  // Eigen::Vector3d &vec = pv.world;
  Bi << 2 * vec(0), 0, 0, vec(1), vec(0), 0, vec(2), 0, vec(0), 0, 2 * vec(1),
      0, 0, vec(2), vec(1), 0, 0, 2 * vec(2);
...
public:
  ...
  void push_voxel(vector<PointCluster> &vec_orig, PointCluster &fix, double coe,
                  Eigen::Vector3d &eig_value, Eigen::Matrix3d &eig_vector,
  ...
  void acc_evaluate2(const vector<IMUST> &xs, int head, int end,
    ...
    vector<Eigen::Matrix<double, 3, 6>,
    ...
    for (int a = head; a < end; a++) {
      ...
      // Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(sig.P/sig.N - vBar
      // * vBar.transpose()); const Eigen::Vector3d &lmbd = saes.eigenvalues();
  ...
  void evaluate_only_residual(const vector<IMUST> &xs, int head, int end,
                              double &residual) {
    residual = 0;
    // vector<PointCluster> sig_tran(win_size);
    ...
    for (int a = head; a < end; a++) {
      ...
      // Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(sig.P - sig.v *
      // vBar.transpose());
...
public:
  ...
  double divide_thread(vector<IMUST> &x_stats, LidarFactor &voxhess,
                       Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT) {
    // int thd_num = 4;
    double residual = 0;
    Hess.setZero();
    ...
    for (int i = 1; i < tthd_num; i++)
      mthreads[i] = new thread(&LidarFactor::acc_evaluate2, &voxhess, x_stats,
                               part * i, part * (i + 1), ref(hessians[i]),
    ...
    for (int i = 0; i < tthd_num; i++) {
      if (i != 0)
        ...
        voxhess.acc_evaluate2(x_stats, 0, part, hessians[0], jacobins[0],
                              resis[0]);
      Hess += hessians[i];
      JacT += jacobins[i];
      residual += resis[i];
  ...
  double only_residual(vector<IMUST> &x_stats, LidarFactor &voxhess) {
    double residual1 = 0;
    // voxhess.evaluate_only_residual(x_stats, 0, voxhess.plvec_voxels.size(),
    // residual1);
    ...
    for (int i = 1; i < thd_num; i++)
      mthreads[i] =
          new thread(&LidarFactor::evaluate_only_residual, &voxhess, x_stats,
                     part * i, part * (i + 1), ref(residuals[i]));

    for (int i = 0; i < thd_num; i++) {
      if (i != 0)
        mthreads[i]->join();
      else
        voxhess.evaluate_only_residual(x_stats, part * i, part * (i + 1),
                                       residuals[i]);
  ...
  bool damping_iter(vector<IMUST> &x_stats, LidarFactor &voxhess,
                    Eigen::MatrixXd *hess, vector<double> &resis,
...
public:
  ...
  void hess_plus(Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT,
    ...
    for (int i = 0; i < win_size; i++) {
      ...
      for (int j = 0; j < win_size; j++)
        Hess.block<DVEL, DVEL>(i * DIM, j * DIM) +=
            hs.block<DVEL, DVEL>(i * DVEL, j * DVEL);
    }
  }

  double divide_thread(vector<IMUST> &x_stats, LidarFactor &voxhess,
                       deque<IMU_PRE *> &imus_factor, Eigen::MatrixXd &Hess,
                       Eigen::VectorXd &JacT) {
    int thd_num = 5;
    double residual = 0;
    Hess.setZero();
    ...
    // cal the imu Hess and jacT
    for (int i = 0; i < win_size - 1; i++) {
      jtj.setZero();
      gg.setZero();
      residual += imus_factor[i]->give_evaluate(x_stats[i], x_stats[i + 1], jtj,
                                                gg, true);
  ...
  double only_residual(vector<IMUST> &x_stats, LidarFactor &voxhess,
    ...
    for (int i = 0; i < win_size - 1; i++)
      residual1 += imus_factor[i]->give_evaluate(x_stats[i], x_stats[i + 1],
                                                 jtj, gg, false);
  ...
  void damping_iter(vector<IMUST> &x_stats, LidarFactor &voxhess,
                    deque<IMU_PRE *> &imus_factor, Eigen::MatrixXd *hess) {
    win_size = voxhess.win_size;
    jac_leng = win_size * 6;
...
public:
  ...
  double divide_thread(vector<IMUST> &x_stats, LidarFactor &voxhess,
    ...
    for (int i = 0; i < win_size - 1; i++) {
      jtj.setZero();
      gg.setZero();
      residual += imus_factor[i]->give_evaluate_g(x_stats[i], x_stats[i + 1],
                                                  jtj, gg, true);
      Hess.block<DIM * 2, DIM * 2>(i * DIM, i * DIM) +=
          jtj.block<2 * DIM, 2 * DIM>(0, 0);
      Hess.block<DIM * 2, 3>(i * DIM, imu_leng - 3) +=
          jtj.block<2 * DIM, 3>(0, 2 * DIM);
      Hess.block<3, DIM * 2>(imu_leng - 3, i * DIM) +=
          jtj.block<3, 2 * DIM>(2 * DIM, 0);
      Hess.block<3, 3>(imu_leng - 3, imu_leng - 3) +=
          jtj.block<3, 3>(2 * DIM, 2 * DIM);
  ...
  double only_residual(vector<IMUST> &x_stats, LidarFactor &voxhess,
    ...
    for (int i = 0; i < win_size - 1; i++)
      residual1 += imus_factor[i]->give_evaluate_g(x_stats[i], x_stats[i + 1],
                                                   jtj, gg, false);
    residual1 *= (imu_coef * 0.5);
  ...
  void damping_iter(vector<IMUST> &x_stats, LidarFactor &voxhess,
                    deque<IMU_PRE *> &imus_factor, vector<double> &resis,
...
struct Keyframe {
  ...
  void generate(pcl::PointCloud<PointType> &pl_send,
...
public:
  ...
  void plane_update() {
    ...
    int l = 0;
    Eigen::Vector3d u[3] = {eig_vector.col(0), eig_vector.col(1),
                            eig_vector.col(2)};
    ...
    for (int k = 0; k < 3; k++)
      if (k != l) {
        ...
        fkl.tail(3) =
            -(u[k].dot(plane.center) * u[l] + u[l].dot(plane.center) * u[k]);
  ...
  int match(Eigen::Vector3d &wld, Plane *&pla, double &max_prob,
      ...
      if (plane.is_plane) {
        ...
        if (range_dis <= 3 * 3 * plane.radius) {
          ...
          if (dis_to_plane < 3 * sqrt(sigma_l)) {
            // float prob = 1 / (sqrt(sigma_l)) * exp(-0.5 * dis_to_plane *
            // dis_to_plane / sigma_l); if(prob > max_prob)
            {
              oc = this;
              sigma_d = sigma_l;
              // max_prob = prob;
  ...
  void tras_display(int win_count, pcl::PointCloud<PointType> &pl_fixd,
                    pcl::PointCloud<PointType> &pl_wind, vector<IMUST> &x_buf) {
    if (octo_state == 0) {
      Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(pcr_add.cov());
...
int match(unordered_map<VOXEL_LOC, OctoTree *> &feat_map, Eigen::Vector3d &wld,
          Plane *&pla, Eigen::Matrix3d &var_wld, double &sigma_d,
          OctoTree *&oc) {
  int flag = 0;
  ...
  if (iter != feat_map.end()) {
    ...
    if (flag && pla == nullptr) {
      printf("pla null max_prob: %lf %ld %ld %ld\n", max_prob, iter->first.x,
             iter->first.y, iter->first.z);
    }
  }
